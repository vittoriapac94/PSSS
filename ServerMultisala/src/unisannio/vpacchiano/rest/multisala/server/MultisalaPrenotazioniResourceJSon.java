...
import unisannio.vpacchiano.rest.multisala.thesystem.PostiTerminatiException;
import unisannio.vpacchiano.rest.multisala.thesystem.PrenotazioneDuplicataException;
import unisannio.vpacchiano.rest.multisala.thesystem.PrenotazioneSconosciutaException;
import unisannio.vpacchiano.rest.multisala.thesystem.ProgrammazioneSconosciutaException;
...
public class MultisalaPrenotazioniResourceJSon extends ServerResource {
	...
	public String getPrenotazione(){
		...
		try {
			...
			status = new Status(Constants.ECCEZIONE_PRENOTAZIONE_INESISTENTE,"ProgrammazioneNotFound","La programmazione non è stata trovatao",null);
			setStatus(status);
	...
	public String addPrenotazione(String body){
		...
		try {
			...
			status = new Status(Constants.ECCEZIONE_PRENOTAZIONE_INESISTENTE,"PrenotazioneNotFound","La prenotazione non è stata trovata",null);
			setStatus(status);
			return gson.toJson(status, Status.class);
			
		} catch (PrenotazioneDuplicataException e) {
			status = new Status(Constants.ECCEZIONE_PRENOTAZIONE_DUPLICATA,"ProgrammazioneDuplicata","La prenotazione è già presente nel database",null);
			setStatus(status);
			return gson.toJson(status, Status.class);
