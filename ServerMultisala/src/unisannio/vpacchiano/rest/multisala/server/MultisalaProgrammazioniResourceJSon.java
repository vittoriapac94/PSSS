...
import unisannio.vpacchiano.rest.multisala.thesystem.ProgrammazioneDuplicataException;
...
public class MultisalaProgrammazioniResourceJSon extends ServerResource {
	...
	public String getProgrammazione(){
		...
		try {
			...
			status = new Status(Constants.ECCEZIONE_PROGRAMMAZIONE_INESISTENTE,"ProgrammazioneNotFound","La programmazione non è stata trovatao",null);
			setStatus(status);
	...
	public String addProgrammazione(String body){
		...
		try {
			...
			status = new Status(Constants.ECCEZIONE_PROGRAMMAZIONE_INESISTENTE,"ProgrammazioneNotFound","La programmazione non è stata trovato",null);
			setStatus(status);
			return gson.toJson(status, Status.class);
			
		} catch (ProgrammazioneDuplicataException e) {
			status = new Status(Constants.ECCEZIONE_PROGRAMMAZIONE_DUPLICATA,"Programmazione Duplicata","La programmazione è già presente nel database",null);
			setStatus(status);
			return gson.toJson(status, Status.class);
	...
	public String deleteProgrammazione(){
		...
		try{
			...
			status = new Status(Constants.ECCEZIONE_PROGRAMMAZIONE_INESISTENTE,"ProgrammazioneNotFound","La programmazione non è stata trovata",null);
			setStatus(status);
			return gson.toJson(status, Status.class);
		}catch (SQLException ex) {
			status = new Status(Constants.ECCEZIONE_COLLEGAMENTO_DATABASE,"DatabaseError","Errore accesso al DataBase",null);
